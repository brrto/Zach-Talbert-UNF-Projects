...
function Character1() {
    ...
    this.updateHealth = function(bullet) {
        ...
        if (character1.hits(bullet) && !bullet.toDelete) {
            if (character2.gunType==="shotgun")
                this.health -= 3;
            if (character2.gunType==="burstShot")
                this.health -= 2;
            if (character2.gunType==="sniper")
                this.health -= 8;
            if (character2.gunType==="laser")
                this.health -= 2;
            playerHit.play();
    ...
    this.checkEdges = function() {
        ...
        if (this.position.x > width/2-120-this.size.x) {
            this.position.x = width/2-120-this.size.x;
            this.healthPosition.x = this.position.x+4;
        }
    }
    ...
    this.checkIfDead = function() {
        if (character1 != null && character1.toDelete) {
            character1 = null;
        }
    }
}